...
See `public/` — `avatar.png`. Favicon: `app/icon.svg`.

## SEO

- Copy `.env.local.example` → `.env.local` and set `NEXT_PUBLIC_SITE_URL` to your production domain (no trailing slash).
- Homepage metadata lives in `lib/data.ts` (`siteMeta`) and `lib/seo.ts`.
- Social preview image is generated by `app/opengraph-image.tsx` (1200×630). To use a static image instead, add `public/og.png` and point `openGraph.images` in `lib/seo.ts`.
- Crawl files: `app/robots.ts`, `app/sitemap.ts`.
